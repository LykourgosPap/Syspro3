...
int main(int argc, char **argv)
{
  ...
  unsigned int    addrlen;            /* argument to accept */
  struct stat stat_buf;      /* argument to fstat */
  off_t offset = 0;          /* file offset */
  char filename[1024];   /* filename to send */
  ...
  while (1) {
    ...
    if (!strncmp(filename, "LIST", 4)) {
        FILE *ls = popen("ls -Rp", "r");
        char buf[512];
        while (fgets(buf, sizeof(buf), ls) != 0) {
            if (buf[0] != '\n')
                write(desc, buf, strlen(buf)+1);
        }
        pclose(ls);
        close(desc);
    }

    if (!strncmp(filename, "FETCH", 5)) {
            fprintf(stderr, "received request to send file %s\n", filename);

            /* open the file to be sent */
            fd = open(&filename[6], O_RDONLY);
            if (fd == -1) {
